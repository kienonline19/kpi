...
class StaffManager:
    def __init__(self, parent_frame, db_manager, update_callback=None):
        ...
        self.education_levels = ["Tiến sĩ", "Thạc sĩ", "Đại học", "Cao đẳng", "Trung cấp", "THPT"]
    ...
    def create_widgets(self):
        ...
        left_panel = ttk.LabelFrame(self.parent_frame, text="Thông Tin Cán Bộ", padding="15")
        ...
        for i, (label, var_name) in enumerate(staff_fields):
            ...
            if var_name == "gender":
                ...
                self.staff_vars[var_name] = tk.StringVar(value=self.positions[0])
                widget = ttk.Combobox(left_panel, textvariable=self.staff_vars[var_name],
                                    values=self.positions, width=32)
            elif var_name == "education":
                self.staff_vars[var_name] = tk.StringVar(value=self.education_levels[0])
                widget = ttk.Combobox(left_panel, textvariable=self.staff_vars[var_name],
                ...
                self.staff_vars[var_name] = tk.StringVar()
                widget = ttk.Entry(left_panel, textvariable=self.staff_vars[var_name], width=35)
            
            widget.grid(row=i, column=1, sticky=(tk.W, tk.E), pady=3, padx=(5, 0))
        
        # Address field
        ttk.Label(left_panel, text="Địa Chỉ:").grid(row=len(staff_fields), column=0, sticky=tk.W, pady=3)
        self.address_text = tk.Text(left_panel, width=35, height=2)
        self.address_text.grid(row=len(staff_fields), column=1, sticky=(tk.W, tk.E), pady=3, padx=(5, 0))
        
        # Staff buttons
        btn_frame = ttk.Frame(left_panel)
        btn_frame.grid(row=len(staff_fields)+1, column=0, columnspan=2, pady=(15, 0))
        
        ttk.Button(btn_frame, text="Thêm", command=self.add_staff).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Cập Nhật", command=self.update_staff).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Xóa", command=self.delete_staff).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Làm Mới", command=self.clear_form).pack(side=tk.LEFT)
        
        # Right panel - Staff list
        right_panel = ttk.LabelFrame(self.parent_frame, text="Danh Sách Cán Bộ", padding="15")
        right_panel.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
        
        # Staff search and filter frame
        ...
        # Staff treeview
        columns = ("Mã CB", "Họ Tên", "Phòng Ban", "Chức Vụ", "SĐT", "Email", "Lương", "Trạng Thái")
    ...
    def update_staff(self):
        ...
        if not selected:
            messagebox.showwarning("Cảnh báo", "Vui lòng chọn cán bộ để cập nhật!")
            return
        ...
        try:
            self.db.update_data('staff', staff_data, {'column': 'staff_code', 'value': staff_code})
            self.refresh_list()
            if self.update_callback:
                self.update_callback()
            self.clear_form()
    ...
    def delete_staff(self):
        ...
        if messagebox.askyesno("Xác nhận", "Bạn có chắc chắn muốn xóa cán bộ này?"):
            try:
                ...
                self.db.delete_data('kpi_assignments', {'column': 'staff_id', 'value': staff_id})
                
                # Delete staff
                self.db.delete_data('staff', {'column': 'staff_code', 'value': staff_code})
    ...
    def validate_input(self):
        """Validate form input"""
        required_fields = ['staff_code', 'full_name', 'department_id', 'position']
    ...
    def on_select(self, event):
        ...
        if selected:
            ...
            if staff_data:
                ...
                self.staff_vars['department_id'].set(staff[17])  # dept_name from join
    ...
    def update_department_comboboxes(self):
        ...
        # Update staff department combobox
        self.dept_combobox['values'] = dept_names
        
        # Update staff filter combobox

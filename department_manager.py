...
class DepartmentManager:
    ...
    def create_widgets(self):
        ...
        left_panel = ttk.LabelFrame(self.parent_frame, text="Thông Tin Phòng Ban", padding="15")
        ...
        for i, (label, var_name) in enumerate(dept_fields):
            ...
            widget = ttk.Entry(left_panel, textvariable=self.dept_vars[var_name], width=35)
            widget.grid(row=i, column=1, sticky=(tk.W, tk.E), pady=3, padx=(5, 0))
        
        # Description field
        ttk.Label(left_panel, text="Mô Tả:").grid(row=len(dept_fields), column=0, sticky=tk.W, pady=3)
        self.dept_description_text = tk.Text(left_panel, width=35, height=3)
        self.dept_description_text.grid(row=len(dept_fields), column=1, sticky=(tk.W, tk.E), pady=3, padx=(5, 0))
        
        # Department buttons
        dept_btn_frame = ttk.Frame(left_panel)
        dept_btn_frame.grid(row=len(dept_fields)+1, column=0, columnspan=2, pady=(15, 0))
        
        ttk.Button(dept_btn_frame, text="Thêm", command=self.add_department).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(dept_btn_frame, text="Cập Nhật", command=self.update_department).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(dept_btn_frame, text="Xóa", command=self.delete_department).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(dept_btn_frame, text="Làm Mới", command=self.clear_form).pack(side=tk.LEFT)
        
        # Right panel - Department list
        right_panel = ttk.LabelFrame(self.parent_frame, text="Danh Sách Phòng Ban", padding="15")
        ...
        ttk.Button(search_frame, text="Làm Mới", command=self.refresh_list).pack(side=tk.RIGHT)
        
        # Department treeview
        columns = ("Mã PB", "Tên Phòng Ban", "Trưởng Phòng", "SĐT", "Email", "Số NV", "Ngân Sách")
    ...
    def update_department(self):
        ...
        if not selected:
            messagebox.showwarning("Cảnh báo", "Vui lòng chọn phòng ban để cập nhật!")
            return
        ...
        try:
            self.db.update_data('departments', dept_data, {'column': 'dept_code', 'value': dept_code})
            self.refresh_list()
            if self.update_callback:
                self.update_callback()
            self.clear_form()
            messagebox.showinfo("Thành công", "Đã cập nhật phòng ban thành công!")
        except Exception as e:
            messagebox.showerror("Lỗi", f"Không thể cập nhật phòng ban: {str(e)}")
    ...
    def delete_department(self):
        ...
        try:
            ...
            # Delete related data
            self.db.execute_query("DELETE FROM kpi_assignments WHERE kpi_id IN (SELECT id FROM kpi WHERE department_id = ?)", [dept_id])
            self.db.execute_query("DELETE FROM kpi_results WHERE kpi_id IN (SELECT id FROM kpi WHERE department_id = ?)", [dept_id])
            self.db.execute_query("DELETE FROM kpi WHERE department_id = ?", [dept_id])
            self.db.execute_query("DELETE FROM staff WHERE department_id = ?", [dept_id])
            self.db.delete_data('departments', {'column': 'dept_code', 'value': dept_code})
            
            self.refresh_list()
            if self.update_callback:
                self.update_callback()
            self.clear_form()
            messagebox.showinfo("Thành công", "Đã xóa phòng ban và dữ liệu liên quan!")
        except Exception as e:

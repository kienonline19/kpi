...
class MainApplication:
    def __init__(self, root):
        ...
        self.kpi_frequencies = ["Hàng ngày", "Hàng tuần",
                                "Hàng tháng", "Hàng quý", "Hàng năm"]
        self.kpi_units = ["VND", "USD", "%",
                          "Số lượng", "Tỷ lệ", "Điểm", "Giờ", "Ngày"]
    ...
    def create_department_tab(self):
        ...
        self.dept_manager = DepartmentManager(
            dept_frame, self.db, self.update_all_comboboxes)
    ...
    def create_staff_tab(self):
        ...
        self.staff_manager = StaffManager(
            staff_frame, self.db, self.update_all_comboboxes)
    ...
    def create_kpi_tab(self):
        ...
        self.kpi_manager = KPIManager(
            kpi_frame, self.db, self.update_all_comboboxes)
    ...
    def create_kpi_assignment_tab(self):
        ...
        top_frame = ttk.LabelFrame(
            assign_frame, text="Phân Công KPI cho Cán Bộ", padding="15")
        top_frame.pack(fill=tk.X, pady=(0, 10))

        assign_form = ttk.Frame(top_frame)
        assign_form.pack(fill=tk.X)

        ttk.Label(assign_form, text="Chọn KPI:").grid(
            row=0, column=0, padx=(0, 5))
        self.assign_kpi_var = tk.StringVar()
        self.assign_kpi_combo = ttk.Combobox(
            assign_form, textvariable=self.assign_kpi_var, width=30)
        self.assign_kpi_combo.grid(row=0, column=1, padx=(0, 10))

        ttk.Label(assign_form, text="Chọn Cán Bộ:").grid(
            row=0, column=2, padx=(0, 5))
        self.assign_staff_var = tk.StringVar()
        self.assign_staff_combo = ttk.Combobox(
            assign_form, textvariable=self.assign_staff_var, width=30)
        self.assign_staff_combo.grid(row=0, column=3, padx=(0, 10))

        ttk.Label(assign_form, text="Vai Trò:").grid(
        ...
        ttk.Button(assign_form, text="Phân Công", command=self.assign_kpi).grid(
            row=0, column=6, padx=(10, 0))
        ttk.Button(assign_form, text="Hủy Phân Công", command=self.unassign_kpi).grid(
            row=0, column=7, padx=(5, 0))

        assign_list_frame = ttk.LabelFrame(
            assign_frame, text="Danh Sách Phân Công KPI", padding="15")
        assign_list_frame.pack(fill=tk.BOTH, expand=True)

        assign_columns = ("KPI", "Tên KPI", "Cán Bộ",
                          "Họ Tên", "Vai Trò", "Ngày Phân Công")
        self.assign_tree = ttk.Treeview(
            assign_list_frame, columns=assign_columns, show='headings', height=20)
        ...
        assign_v_scroll = ttk.Scrollbar(
            assign_list_frame, orient=tk.VERTICAL, command=self.assign_tree.yview)
    ...
    def create_kpi_results_tab(self):
        ...
        top_frame = ttk.LabelFrame(
            results_frame, text="Nhập Kết Quả KPI", padding="15")
        top_frame.pack(fill=tk.X, pady=(0, 10))
        ...
        for i, (label, var_name) in enumerate(results_fields):
            ttk.Label(top_frame, text=label).grid(
                row=i//2, column=(i % 2)*2, sticky=tk.W, padx=(0, 5), pady=3)

            if var_name == "kpi_id":
                self.results_vars[var_name] = tk.StringVar()
                widget = ttk.Combobox(
                    top_frame, textvariable=self.results_vars[var_name], width=30)
                self.results_kpi_combo = widget
            elif var_name == "period":
                self.results_vars[var_name] = tk.StringVar(
                    value=datetime.now().strftime("%Y-%m"))
                widget = ttk.Entry(
                    top_frame, textvariable=self.results_vars[var_name], width=32)
            else:
                self.results_vars[var_name] = tk.StringVar()
                widget = ttk.Entry(
                    top_frame, textvariable=self.results_vars[var_name], width=32)

            widget.grid(row=i//2, column=(i % 2)*2+1,
                        sticky=(tk.W, tk.E), padx=(0, 20), pady=3)

        results_btn_frame = ttk.Frame(top_frame)
        results_btn_frame.grid(row=2, column=0, columnspan=4, pady=(15, 0))

        ttk.Button(results_btn_frame, text="Lưu Kết Quả",
                   command=self.save_kpi_result).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(results_btn_frame, text="Làm Mới",
                   command=self.clear_results_form).pack(side=tk.LEFT)

        results_list_frame = ttk.LabelFrame(
            results_frame, text="Lịch Sử Kết Quả KPI", padding="15")
        ...
        results_filter_combo = ttk.Combobox(
            filter_frame, textvariable=self.results_filter_var, width=30)
        ...
        ttk.Button(filter_frame, text="Làm Mới", command=self.refresh_results_list).grid(
            row=0, column=2, padx=(10, 0))

        results_columns = ("KPI", "Tên KPI", "Kỳ", "Mục Tiêu",
                           "Thực Tế", "Đạt (%)", "Ghi Chú", "Ngày Ghi")
        self.results_tree = ttk.Treeview(
            results_list_frame, columns=results_columns, show='headings', height=20)
        ...
        results_v_scroll = ttk.Scrollbar(
            results_list_frame, orient=tk.VERTICAL, command=self.results_tree.yview)
    ...
    def create_admin_tab(self):
        ...
        categories_frame = ttk.LabelFrame(
            admin_frame, text="Quản Lý Danh Mục KPI", padding="15")
        categories_frame.pack(fill=tk.X, pady=(0, 10))

        cat_form_frame = ttk.Frame(categories_frame)
        cat_form_frame.pack(fill=tk.X)

        ttk.Label(cat_form_frame, text="Tên Danh Mục:").grid(
            row=0, column=0, padx=(0, 5))
        self.category_name_var = tk.StringVar()
        ttk.Entry(cat_form_frame, textvariable=self.category_name_var,
                  width=30).grid(row=0, column=1, padx=(0, 10))

        ttk.Label(cat_form_frame, text="Mô Tả:").grid(
            row=0, column=2, padx=(0, 5))
        self.category_desc_var = tk.StringVar()
        ttk.Entry(cat_form_frame, textvariable=self.category_desc_var,
                  width=40).grid(row=0, column=3, padx=(0, 10))

        ttk.Button(cat_form_frame, text="Thêm Danh Mục",
                   command=self.add_category).grid(row=0, column=4, padx=(10, 0))
        ...
        self.category_tree = ttk.Treeview(
            cat_list_frame, columns=cat_columns, show='headings', height=8)

        for col in cat_columns:
            self.category_tree.heading(col, text=col)
            self.category_tree.column(col, width=150, minwidth=100)

        cat_v_scroll = ttk.Scrollbar(
            cat_list_frame, orient=tk.VERTICAL, command=self.category_tree.yview)
        ...
        stats_frame = ttk.LabelFrame(
            admin_frame, text="Thống Kê Cơ Sở Dữ Liệu", padding="15")
        stats_frame.pack(fill=tk.BOTH, expand=True)

        self.stats_text = tk.Text(
            stats_frame, wrap=tk.WORD, font=('Consolas', 10), height=15)
        ...
        ttk.Button(admin_frame, text="Cập Nhật Thống Kê",
                   command=self.update_database_stats).pack(pady=10)
    ...
    def assign_kpi(self):
        ...
        kpi_id = self.db.execute_query(
            "SELECT id FROM kpi WHERE kpi_code = ?", [kpi_code])[0][0]
        staff_id = self.db.execute_query(
            "SELECT id FROM staff WHERE staff_code = ?", [staff_code])[0][0]
    ...
    def unassign_kpi(self):
        ...
        if not selected:
            messagebox.showwarning(
                "Cảnh báo", "Vui lòng chọn phân công để hủy!")
            return
        ...
        if messagebox.askyesno("Xác nhận", "Bạn có chắc chắn muốn hủy phân công này?"):
            try:
                ...
                messagebox.showerror(
                    "Lỗi", f"Không thể hủy phân công: {str(e)}")
    ...
    def refresh_assignment_list(self):
        ...
        for row in results:
            ...
            self.assign_tree.insert(
                "", "end", values=row[:5] + (assigned_date,))
    ...
    def save_kpi_result(self):
        ...
        achievement_percentage = (
            actual_value / target_value * 100) if target_value > 0 else 0
    ...
    def update_all_comboboxes(self):
        ...
        if hasattr(self, 'assign_kpi_combo'):
            kpi_displays = [f"{row[0]} - {row[1]}" for row in self.db.execute_query(
                "SELECT kpi_code, kpi_name FROM kpi WHERE status = 'active' ORDER BY kpi_code")]
            self.assign_kpi_combo['values'] = kpi_displays

        if hasattr(self, 'assign_staff_combo'):
            staff_displays = [f"{row[0]} - {row[1]}" for row in self.db.execute_query(
                "SELECT staff_code, full_name FROM staff WHERE status = 'active' ORDER BY staff_code")]

...
class KPIManager:
    ...
    def create_widgets(self):
        ...
        left_panel = ttk.LabelFrame(
            self.parent_frame, text="Thông Tin KPI", padding="15")
        ...
        for i, (label, var_name) in enumerate(kpi_fields):
            ...
            if var_name == "category_id":
                ...
                self.kpi_vars[var_name] = tk.StringVar()
                widget = ttk.Entry(
                    left_panel, textvariable=self.kpi_vars[var_name], width=35)

            widget.grid(row=i, column=1, sticky=(
                tk.W, tk.E), pady=3, padx=(5, 0))

        ttk.Label(left_panel, text="Mô Tả:").grid(
            row=len(kpi_fields), column=0, sticky=tk.W, pady=3)
        self.description_text = tk.Text(left_panel, width=35, height=3)
        self.description_text.grid(
            row=len(kpi_fields), column=1, sticky=(tk.W, tk.E), pady=3, padx=(5, 0))

        btn_frame = ttk.Frame(left_panel)
        btn_frame.grid(row=len(kpi_fields)+1, column=0,
                       columnspan=2, pady=(15, 0))

        ttk.Button(btn_frame, text="Thêm", command=self.add_kpi).pack(
            side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Cập Nhật", command=self.update_kpi).pack(
            side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Xóa", command=self.delete_kpi).pack(
            side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Làm Mới",
                   command=self.clear_form).pack(side=tk.LEFT)

        right_panel = ttk.LabelFrame(
            self.parent_frame, text="Danh Sách KPI", padding="15")
        ...
        columns = ("Mã KPI", "Tên KPI", "Phòng Ban", "Đơn Vị",
                   "Mục Tiêu", "Trọng Số", "Tần Suất", "Trạng Thái")
    ...
    def update_kpi(self):
        ...
        if not selected:
            messagebox.showwarning(
                "Cảnh báo", "Vui lòng chọn KPI để cập nhật!")
            return
        ...
        try:
            self.db.update_data(
                'kpi', kpi_data, {'column': 'kpi_code', 'value': kpi_code})
            self.refresh_list()
            if self.update_callback:
                self.update_callback()
            self.clear_form()
    ...
    def delete_kpi(self):
        ...
        if messagebox.askyesno("Xác nhận", "Bạn có chắc chắn muốn xóa KPI này?"):
            try:
                ...
                self.db.delete_data(
                    'kpi_results', {'column': 'kpi_id', 'value': kpi_id})
                self.db.delete_data('kpi_assignments', {
                                    'column': 'kpi_id', 'value': kpi_id})
                self.db.delete_data(
                    'kpi', {'column': 'kpi_code', 'value': kpi_code})
                ...
                self.clear_form()
                messagebox.showinfo(
                    "Thành công", "Đã xóa KPI và dữ liệu liên quan!")
            except Exception as e:
    ...
    def on_select(self, event):
        ...
        if selected:
            ...
            if kpi_data:
                ...
                self.kpi_vars['category_id'].set(kpi[13] or "")
                self.kpi_vars['department_id'].set(kpi[14] or "")
    ...
    def update_comboboxes(self):
        ...
        cat_names = [row[0] for row in self.db.execute_query(
            "SELECT category_name FROM kpi_categories ORDER BY category_name")]
